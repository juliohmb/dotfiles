...
while running:
    ...
    if current_position[1] + len(current_piece) >= SCREEN_HEIGHT or \
        any(screen.get_at((int(current_position[0] + x), int(current_position[1] + y))) != BLACK
        ...
        for y, row in enumerate(current_piece):
            for x, cell in enumerate(row):
                if cell != 0:
                    screen.set_at((int(current_position[0] + x), int(current_position[1] + y)), cell)
    ...
    for y, row in enumerate(current_piece):
        for x, cell in enumerate(row):
            if cell != 0:
                rect = pygame.Rect(int(current_position[0] + x), int(current_position[1] + y), 1, 1)
